...
因此，如果解码没有选择正确的对应编码，那么就不能正确翻译被编码的字节。
...
每种语言都在发展中演进除了自己的安全的面向 UTF-8 的遍历方式。
...
| 编码安全措施（运行时） | 无，任何 byte 序列都能编译通过；出现非 ASCII 时程序员自己保证多字节边界 | 字符串构造时已保证 UTF-16 合法；如果 `new String(bytes, UTF_8)` 遇到非法序列，JDK 默认用 `?` 替换 | `range` 在解码 UTF-8 时同步检查：遇到非法序列会替换成 `utf8.RuneError`（U+FFFD）并继续，不会 panic | `str` 本身必须是合法 UTF-8，构造时已验证；`bytes()` 只是裸读，不会断字，但程序员自己负责后续解码 |
