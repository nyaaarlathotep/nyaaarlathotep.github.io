...
可以通过原生，底层一点的方法，手动获取连接，手动commit来提交事务，也可以通过Spring已经为我们封装好的，给类或者方法加上注解`@Transactional`，来为方法开启事务支持。

`@Transactional`是通过AOP实现的，所以也会因此有一些特殊的规则与表现，没能完全对使用者透明。在我们的使用中，会发现一些情况下事务托管不生效了，这些情况也是和Spring和AOP息息相关的。

# 现象

这是一个关于paper更新的方法，以各种不同姿势对它做了一些更改，以模拟`@Transactional`并复现发生的各种情况。
...
总之，这个事务不生效的问题，归根结底是一个aop没有生效的问题，是一个Spring托管bean的问题，没有对Spring以及AOP，代理的理解，就无法理解这个问题。
