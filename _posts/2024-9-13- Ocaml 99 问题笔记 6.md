...
### 73. Node Degree and Graph Coloration
...
### 74. Depth-First Order Graph Traversal
...
### 74. Generate K-Regular Simple Graphs With N Nodes
...
let generate node_num regular = 
	...
	in
	let rec aux now next edges = if next>6 && List.length edges = (regular * node_num) 
		then [edges] 
			else if next>6 then [] 
		else
		if count_edges now = regular then 
		if List.length edges = 3 then aux (now+1) (now+2) (edges @ other_edges) else
		if count_edges next <= 3 then 
