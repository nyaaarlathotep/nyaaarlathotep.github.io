...
查了查没太找到，要不直接暴力得了。不行了，太麻烦了，想要稍微剪枝一下都要很麻烦的回溯和递归，还是暴力搞吧。

```
let graphg_iso g1 g2 = 
	if List.length g1.nodes != List.length g2.nodes || List.length g1.edges != List.length g2.nodes then false
	else
	let rec match_node a remain_g2_nodes map= match remain_g2_nodes with
		| [] -> false
		| h::t -> let a_nei = neighbors a in let h_nei = neighbors h in
			if List.length a_nei ! List.length h_nei then false
			else 
	in
	let rec match_g1 remain_g1_nodes remain_g2_nodes = 
