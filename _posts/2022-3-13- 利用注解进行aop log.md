...
一般来说，Spring boot项目的所有Controller调用，入参要统一进行log记录，也可以纪录方法花费的时间，定位问题，我们可以借助aop以及反射，将其统一处理，不需要每一个方法都写一遍log。这可以说是aop最典型的实践。

在此基础之上，还可以再将其进一步简化，利用注解，只要加了注解的方法都可以进行log，更加简洁。
...
当Controller中的方法需要log的时候，加上注解，就会在受到调用的时候自动记录相关信息。
...
具体的记录过程，通过反射获取method的相关信息，包括方法名称，参数，等等，在执行方法完成后记录响应结果与花费时间。

如果有其他需要也可以自行增加Log内容其实有些`HandlerMethodArgumentResolver`的工作也能放在这里做，也能实现但不是很合适，还是放在它应该在的地方吧。
